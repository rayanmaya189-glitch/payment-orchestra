...
//! Uses async_trait for clean abstraction over in-memory and SeaORM implementations.
...
pub struct InMemoryOperatorRepository {
    operators: Arc<RwLock<HashMap<Uuid, Operator>>>,
    trade_license_index: Arc<RwLock<HashMap<String, Uuid>>>,
...
impl OperatorRepository for InMemoryOperatorRepository {
    ...
    async fn save(&self, operator: &mut Operator) -> Result<(), OperatorError> {
        ...
        // If operator exists, remove old indexes before updating
    ...
    async fn find_by_subdomain(&self, _subdomain: &str) -> Result<Option<Operator>, OperatorError> {
        // TODO: Implement subdomain index
        Ok(None)
    }
