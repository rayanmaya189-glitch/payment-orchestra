//! Payment intent command handlers.
...
impl<R: OrchestrationRepository + Send + Sync> OrchestrationCommandHandler<R> {
    ...
    pub(crate) async fn authorize_payment_intent_impl(&self, cmd: AuthorizePaymentIntent) -> Result<PaymentIntentResult, OrchestrationError> {
    ...
    pub(crate) async fn capture_payment_intent_impl(&self, cmd: CapturePaymentIntent) -> Result<PaymentIntentResult, OrchestrationError> {
        let mut intent = self.repo.load_payment_intent(cmd.payment_intent_id).await?
            .ok_or(OrchestrationError::NotFound(cmd.payment_intent_id))?;
    ...
    pub(crate) async fn void_payment_intent_impl(&self, cmd: VoidPaymentIntent) -> Result<PaymentIntentResult, OrchestrationError> {
        let mut intent = self.repo.load_payment_intent(cmd.payment_intent_id).await?
            .ok_or(OrchestrationError::NotFound(cmd.payment_intent_id))?;
    ...
    pub(crate) async fn refund_payment_intent_impl(&self, cmd: RefundPaymentIntent) -> Result<PaymentIntentResult, OrchestrationError> {
        let mut intent = self.repo.load_payment_intent(cmd.payment_intent_id).await?
            .ok_or(OrchestrationError::NotFound(cmd.payment_intent_id))?;
