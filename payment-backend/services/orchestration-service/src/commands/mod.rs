...
/// Authorize a PaymentIntent against an acquirer
#[derive(Debug, Clone)]
...
/// Capture (settle) an authorized PaymentIntent
#[derive(Debug, Clone)]
pub struct CapturePaymentIntent {
    pub payment_intent_id: Uuid,
    pub amount_minor_units: Option<i64>, // None = full capture
    pub supports_partial_capture: bool,
    pub max_partial_captures: u32,
    pub actor_id: Uuid,
}

/// Void an authorized (uncaptured) PaymentIntent
#[derive(Debug, Clone)]
...
/// Store a payment method token from an acquirer
#[derive(Debug, Clone)]
...
/// Expire a payment method token
#[derive(Debug, Clone)]
pub struct ExpirePaymentMethodToken {
    pub token_id: Uuid,
}

/// Revoke a payment method token
#[derive(Debug, Clone)]
...
pub struct OrchestrationCommandHandler<R: PaymentIntentRepository + RoutingPolicyRepository + PaymentMethodTokenRepository + IdempotencyCache + AcquirerLinkProvider> {
    repo: R,
}

impl<R: PaymentIntentRepository + RoutingPolicyRepository + PaymentMethodTokenRepository + IdempotencyCache + AcquirerLinkProvider> OrchestrationCommandHandler<R> {
...
#[async_trait]
impl<R: PaymentIntentRepository + RoutingPolicyRepository + PaymentMethodTokenRepository + IdempotencyCache + AcquirerLinkProvider + Send + Sync> CommandHandler for OrchestrationCommandHandler<R> {
    async fn create_payment_intent(&self, cmd: CreatePaymentIntent) -> Result<PaymentIntentResult, OrchestrationError> {
        // 1. Idempotency check
        ...
        match self.repo.check_idempotency(&idem_key).await? {
            ...
            IdempotencyResult::New => {} // Continue
        }
    ...
    async fn authorize_payment_intent(&self, cmd: AuthorizePaymentIntent) -> Result<PaymentIntentResult, OrchestrationError> {
        ...
        // 5. Simulate routing + authorization (in production, this calls connector-gateway)
        ...
        match policy.select_route(card_scheme, &intent.currency, intent.requested_amount.amount_minor_units, &attempted_hops, &available_links) {
            ...
            Err(_) if attempted_hops.is_empty() => {
                // No routes available at all (first attempt with no hops yet)
                // This case shouldn't happen if we have available links, but handle gracefully
                Err(OrchestrationError::NoEligibleRoute)
            }
            Err(_) => {
                // All routes attempted and failed
                let attempt_evt = PaymentAuthorizationAttempted {
                    payment_intent_id: cmd.payment_intent_id,
                    attempt_id,
                    attempt_number,
    ...
    async fn capture_payment_intent(&self, cmd: CapturePaymentIntent) -> Result<PaymentIntentResult, OrchestrationError> {
        ...
        // 3. Check INV-01: captured <= authorized
        intent.check_capture_invariant(&capture_amount)?;

        // 4. Check partial capture support
    ...
    async fn activate_routing_policy(&self, cmd: ActivateRoutingPolicy) -> Result<RoutingPolicyResult, OrchestrationError> {
        ...
        // Deactivate existing policy if any
        ...
        // Compute a simple hash of the rules for the event
