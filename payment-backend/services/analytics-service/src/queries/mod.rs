...
impl<R: AnalyticsRepository + Send + Sync> QueryHandler for AnalyticsQueryHandler<R> {
    async fn authorization_rates(&self, query: AuthRateQuery) -> Result<Vec<AuthRateRow>, AnalyticsError> {
        ...
        // Filter by acquirer and card scheme if specified
        let mut rows: Vec<AuthRateRow> = Vec::new();

        // Group by hour + acquirer + card scheme
    ...
    async fn chargeback_trends(&self, query: ChargebackTrendQuery) -> Result<Vec<ChargebackTrendRow>, AnalyticsError> {
        ...
        // Count chargebacks and total transactions per card scheme
        ...
        for event in &all_events {
            if let Some(ref scheme) = event.card_scheme {
                // Only count payment events for total transactions
                match event.event_type.as_str() {
    ...
    async fn scheme_compliance(&self) -> Result<Vec<SchemeComplianceRow>, AnalyticsError> {
        // For Phase 1: compute from in-memory data with default thresholds
        // In production: query ClickHouse materialized views
        let now = Utc::now();
        ...
        // Group by card scheme
        let mut by_scheme: std::collections::HashMap<
            String,
            (u64, u64, u64),
        > = std::collections::HashMap::new();
        ...
        for event in &all_events {
            if let Some(ref scheme) = event.card_scheme {
                ...
                match event.event_type.as_str() {
                    "PaymentAuthorized" => entry.0 += 1, // authorized
                    "PaymentFailed" => entry.1 += 1,     // declined
                    _ => {}
                }
            }
        }
        ...
        for event in chargebacks {
            if let Some(ref scheme) = event.card_scheme {
                let entry = by_scheme.entry(scheme.clone()).or_insert((0, 0, 0));
                entry.2 += 1; // chargeback count
            }
        }

        // Visa threshold: 1.5% chargeback rate, Mastercard: 1.8%
    ...
    async fn fraud_analysis(&self, query: FraudAnalysisQuery) -> Result<Vec<FraudAnalysisRow>, AnalyticsError> {
        ...
        for event in &all_events {
            ...
            entry.0 += 1; // total transactions

            // Determine if event is fraud: high fraud score OR chargeback event
            let is_fraud = event
                .fraud_score
                .map(|s| s > 0.8)
                .unwrap_or(false)
    ...
    async fn revenue_recovery(&self, date_range: DateRangeFilter) -> Result<Vec<RevenueRecoveryRow>, AnalyticsError> {
        ...
        // Count failover-routed transactions and estimate savings
        ...
        // Estimated savings: assume 2% fee difference on failover routes
