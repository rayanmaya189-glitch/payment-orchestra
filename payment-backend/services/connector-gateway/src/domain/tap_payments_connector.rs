//! Tap Payments Connector — real HTTP adapter for Tap Payments' REST API.
...
impl TapPaymentsConnector {
    ...
    fn normalize_authorize_response(&self, body: &Value, latency_ms: u32) -> Result<AuthorizeResponse, ConnectorError> {
        ...
        let authorize_status = match status { "AUTHORIZED" | "CAPTURED" => AuthorizeStatus::Approved, "DECLINED" | "FAILED" | "CANCELLED" => AuthorizeStatus::Declined, "REDIRECT" => AuthorizeStatus::Requires3DS, _ => AuthorizeStatus::Declined };
        let decline_code = body["response_code"].as_str().map(|code| self.decline_table.normalize(code));
        let three_ds_data = if authorize_status == AuthorizeStatus::Requires3DS {
            Some(ThreeDsData { three_ds_version: "2.0".into(), acs_url: body["transaction_url"].as_str().map(String::from), pareq: None, md: None, session_data: None })
        } else { None };
        Ok(AuthorizeResponse { status: authorize_status, acquirer_reference: Some(id.to_string()), decline_reason: decline_code, approved_amount: body.get("amount").and_then(|a| a.as_f64()).map(|a| Money { amount_minor_units: (a * 100.0) as i64, currency: body["currency"].as_str().unwrap_or("AED").to_uppercase() }), three_ds_data, latency_ms })
    }
}

#[async_trait]
impl AcquirerConnector for TapPaymentsConnector {
    fn connector_id(&self) -> &str { "tap_payments" }
    fn capabilities(&self) -> ConnectorCapabilities {
        ConnectorCapabilities { supports_partial_capture: true, supports_partial_refund: true, supports_native_idempotency_key: true, supports_webhook_settlement: true, supports_realtime_status_check: true, supports_fx_conversion: false, supported_card_schemes: vec![CardScheme::Visa, CardScheme::Mastercard, CardScheme::Amex, CardScheme::Other("Mada".into())], supported_currencies: vec!["AED".into(), "USD".into(), "EUR".into(), "GBP".into(), "SAR".into(), "QAR".into(), "BHD".into(), "KWD".into()], settlement_format: SettlementFormat::Webhook, settlement_cycle: SettlementCycle::NextDay, cross_border_fee_bps: 120 }
    }
    fn onboarding_schema(&self) -> OnboardingSchema {
        OnboardingSchema { connector_id: "tap_payments".into(), fields: vec![ OnboardingField { name: "secret_key".into(), field_type: FieldType::Password, required: true, label: "Secret Key".into(), validation_regex: None, help_text: Some("Find in Tap Dashboard > Settings > API Keys".into()) }, OnboardingField { name: "webhook_secret".into(), field_type: FieldType::Password, required: false, label: "Webhook Secret".into(), validation_regex: None, help_text: None }, OnboardingField { name: "environment".into(), field_type: FieldType::Select { options: vec![SelectOption { value: "sandbox".into(), label: "Sandbox".into() }, SelectOption { value: "production".into(), label: "Production".into() }] }, required: true, label: "Environment".into(), validation_regex: None, help_text: None } ] }
    }

    async fn authorize(&self, req: AuthorizeRequest) -> Result<AuthorizeResponse, ConnectorError> {
        let start = std::time::Instant::now();
        { let mut cb = self.circuit_breaker.lock().map_err(|e| ConnectorError::NetworkError(format!("CB lock: {}", e)))?; if !cb.is_call_allowed() { return Err(ConnectorError::CircuitBreakerOpen("tap_payments".into())); } }
        let payload = serde_json::json!({ "amount": req.amount.amount_minor_units as f64 / 100.0, "currency": req.currency, "payment_method": { "type": "CARD", "card_token_id": req.payment_method_token }, "save_card": false });
        let mut request = self.client.post(format!("{}/charges", self.base_url)).header("Authorization", self.auth_header()).header("Content-Type", "application/json");
        if !req.idempotency_key.is_empty() { request = request.header("Idempotency-Key", &req.idempotency_key); }
        let resp = request.json(&payload).send().await.map_err(|e| { if e.is_timeout() { ConnectorError::Timeout(start.elapsed().as_millis() as u64) } else { ConnectorError::NetworkError(format!("Tap request: {}", e)) } })?;
        let latency_ms = start.elapsed().as_millis() as u32;
        let status_code = resp.status();
        let body: Value = resp.json().await.map_err(|e| ConnectorError::NetworkError(format!("Tap parse: {}", e)))?;
        if status_code.is_success() { let result = self.normalize_authorize_response(&body, latency_ms)?; { let mut cb = self.circuit_breaker.lock().map_err(|e| ConnectorError::NetworkError(format!("CB lock: {}", e)))?; if result.status == AuthorizeStatus::Approved { cb.record_success(); } else { cb.record_failure(); } } Ok(result) } else { let error_msg = body["message"].as_str().unwrap_or("Unknown Tap error"); { let mut cb = self.circuit_breaker.lock().map_err(|e| ConnectorError::NetworkError(format!("CB lock: {}", e)))?; cb.record_failure(); } match status_code.as_u16() { 401 => Err(ConnectorError::AuthenticationFailed(error_msg.into())), 429 => Err(ConnectorError::RateLimited), _ => Err(ConnectorError::AcquirerDeclined(error_msg.into())) } }
    }
    ...
    async fn capture(&self, req: CaptureRequest) -> Result<CaptureResponse, ConnectorError> {
        ...
        let resp = self.client.post(format!("{}/captures/{}", self.base_url, req.acquirer_reference)).header("Authorization", self.auth_header()).header("Content-Type", "application/json").json(&payload).send().await.map_err(|e| ConnectorError::NetworkError(format!("Tap capture: {}", e)))?;
        let latency_ms = start.elapsed().as_millis() as u32;
        let status_code = resp.status();
        let body: Value = resp.json().await.map_err(|e| ConnectorError::NetworkError(format!("Tap parse: {}", e)))?;
        Ok(CaptureResponse { success: status_code.is_success(), acquirer_reference: body["id"].as_str().map(String::from), amount_captured: Money { amount_minor_units: body["amount"].as_f64().map(|a| (a * 100.0) as i64).unwrap_or(req.amount.amount_minor_units), currency: req.amount.currency }, latency_ms })
    }

    async fn void(&self, req: VoidRequest) -> Result<VoidResponse, ConnectorError> {
        let start = std::time::Instant::now();
        let resp = self.client.post(format!("{}/void/{}", self.base_url, req.acquirer_reference)).header("Authorization", self.auth_header()).header("Content-Type", "application/json").send().await.map_err(|e| ConnectorError::NetworkError(format!("Tap void: {}", e)))?;
        let latency_ms = start.elapsed().as_millis() as u32;
        let status_code = resp.status();
        let body: Value = resp.json().await.map_err(|e| ConnectorError::NetworkError(format!("Tap parse: {}", e)))?;
    ...
    async fn refund(&self, req: RefundRequest) -> Result<RefundResponse, ConnectorError> {
        let start = std::time::Instant::now();
        let payload = serde_json::json!({ "charge_id": req.acquirer_reference, "amount": req.amount.amount_minor_units as f64 / 100.0, "currency": req.amount.currency });
        let resp = self.client.post(format!("{}/refunds", self.base_url)).header("Authorization", self.auth_header()).header("Content-Type", "application/json").json(&payload).send().await.map_err(|e| ConnectorError::NetworkError(format!("Tap refund: {}", e)))?;
        let latency_ms = start.elapsed().as_millis() as u32;
        let status_code = resp.status();
        let body: Value = resp.json().await.map_err(|e| ConnectorError::NetworkError(format!("Tap parse: {}", e)))?;
    ...
    async fn status_check(&self, req: StatusCheckRequest) -> Result<StatusCheckResponse, ConnectorError> {
        let start = std::time::Instant::now();
        let resp = self.client.get(format!("{}/charges/{}", self.base_url, req.acquirer_reference)).header("Authorization", self.auth_header()).send().await.map_err(|e| ConnectorError::NetworkError(format!("Tap status: {}", e)))?;
        let latency_ms = start.elapsed().as_millis() as u32;
        let body: Value = resp.json().await.map_err(|e| ConnectorError::NetworkError(format!("Tap parse: {}", e)))?;
        let status = body["status"].as_str().unwrap_or("unknown");
        let auth_status = match status { "AUTHORIZED" | "CAPTURED" => AuthorizeStatus::Approved, "DECLINED" | "FAILED" | "CANCELLED" => AuthorizeStatus::Declined, _ => AuthorizeStatus::Declined };
        Ok(StatusCheckResponse { status: auth_status, acquirer_reference: body["id"].as_str().map(String::from), amount: body.get("amount").and_then(|a| a.as_f64()).map(|a| Money { amount_minor_units: (a * 100.0) as i64, currency: body["currency"].as_str().unwrap_or("AED").to_uppercase() }), latency_ms })
    }

    async fn get_fx_rate(&self, _req: FxRateRequest) -> Result<FxRateResponse, ConnectorError> { Err(ConnectorError::UnsupportedOperation("FX not supported by Tap".into())) }
    ...
    async fn provision_network_token(&self, _req: ProvisionTokenRequest) -> Result<ProvisionTokenResponse, ConnectorError> { Err(ConnectorError::UnsupportedOperation("Network tokens not supported by Tap".into())) }
    async fn account_updater(&self, _token: &NetworkTokenReference) -> Result<AccountUpdateResult, ConnectorError> { Err(ConnectorError::UnsupportedOperation("Account updater not supported by Tap".into())) }
    ...
    async fn validate_credentials(&self, config: &ConnectorConfig) -> Result<CredentialValidationResult, ConnectorError> {
        ...
        let resp = self.client.get(format!("{}/merchant", self.base_url)).header("Authorization", format!("Bearer {}", secret_key)).send().await.map_err(|e| ConnectorError::NetworkError(format!("Tap validate: {}", e)))?;
        ...
        let body: Value = resp.json().await.map_err(|e| ConnectorError::NetworkError(format!("Tap parse: {}", e)))?;
        Ok(CredentialValidationResult { valid: true, merchant_name: body["company"]["name"].as_str().map(String::from), permissions: vec!["authorize".into(), "capture".into(), "refund".into()], error_message: None })
    }
