    ...
    Q: QueryHandler + Send + Sync + 'static,
{
    ...
    async fn update_gateway_profile(
        ...
        let card_schemes: Option<Vec<CardScheme>> = {
            let schemes: Vec<CardScheme> = req.enabled_card_schemes.iter()
                .filter_map(|s| card_scheme_from_str(s))
                .collect();
            ...
            Ok(connectors) => {
                let proto_connectors: Vec<ConnectorInfoView> = connectors
                    ...
                    .map(|c| ConnectorInfoView {
                        ...
                        display_name: connector_display_name(&c.connector_id),
    ...
    async fn test_connection(
        ...
        // Look up the connector from the registry and test the connection
...
fn profile_to_view(profile: GatewayProfile) -> GatewayProfileView {
...
fn card_scheme_from_str(s: &str) -> Option<CardScheme> {
...
fn connector_display_name(connector_id: &str) -> String {
    match connector_id {
        "stripe" => "Stripe".into(),
