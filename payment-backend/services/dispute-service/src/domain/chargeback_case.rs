//! ChargebackCase aggregate — event-sourced dispute management.
//!
//! Core aggregate with representment lifecycle: Received → UnderReview →
//! RepresentmentSubmitted → Won | Lost | Accepted | Escalated.
...
use super::status::ChargebackStatus;
...
impl ChargebackCase {
    ...
    pub fn new(
        ...
        let now = Utc::now();
        let deadline = now + Duration::days(30); // Visa standard
