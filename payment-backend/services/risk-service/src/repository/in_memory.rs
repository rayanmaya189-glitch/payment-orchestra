//! In-memory Risk Scoring repository — BC-11
...
use crate::domain::*;
use crate::repository::traits::RiskRepository;

#[derive(Clone)]
pub struct InMemoryRiskRepository {
    pub(super) assessments: Arc<RwLock<HashMap<Uuid, RiskAssessment>>>,
...
impl RiskRepository for InMemoryRiskRepository {
    ...
    async fn find_high_risk(&self, _operator_id: Uuid, since: DateTime<Utc>) -> Result<Vec<RiskAssessment>, RiskError> {
        ...
        let results: Vec<RiskAssessment> = map
            .values()
            .filter(|a| a.assessed_at >= since && (a.risk_level == RiskLevel::High || a.risk_level == RiskLevel::Critical))
            .cloned()
            .collect();
        Ok(results)
    }
    ...
    async fn get_risk_stats(&self, _operator_id: Uuid, window_hours: u32) -> Result<RiskStats, RiskError> {
        ...
        let windowed: Vec<&RiskAssessment> = map.values().filter(|a| a.assessed_at >= cutoff).collect();
        ...
        let high_count = windowed.iter().filter(|a| a.risk_level == RiskLevel::High).count() as u32;
        let critical_count = windowed.iter().filter(|a| a.risk_level == RiskLevel::Critical).count() as u32;

        Ok(RiskStats {
            avg_risk_score: total_score / windowed.len() as f64,
            high_risk_count: high_count,
            critical_risk_count: critical_count,
            risk_by_bin: HashMap::new(),
            risk_by_country: HashMap::new(),
        })
    }
}
