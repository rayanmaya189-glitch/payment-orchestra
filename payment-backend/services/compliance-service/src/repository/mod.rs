...
pub struct InMemoryComplianceRepository {
    kyb_cases: Arc<RwLock<HashMap<Uuid, KybCase>>>,
    operator_index: Arc<RwLock<HashMap<Uuid, Uuid>>>, // operator_id → kyb_case_id
    aml_alerts: Arc<RwLock<HashMap<Uuid, AmlAlert>>>,
    recent_txns: Arc<RwLock<Vec<RecentTransaction>>>,
}
