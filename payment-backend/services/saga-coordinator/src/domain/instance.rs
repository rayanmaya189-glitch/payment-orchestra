//! SagaInstance — durable state machine aggregate root.
...
use super::saga_type::SagaType;
use super::status::SagaStatus;
use super::step::{SagaStep, StepStatus};
...
impl SagaInstance {
    ...
    pub fn complete_step(&mut self, step_index: usize, output: String) -> Result<(), SagaError> {
        ...
        if self.steps.iter().all(|s| {
            s.status == StepStatus::Succeeded || s.status == StepStatus::Compensated
        }) {
