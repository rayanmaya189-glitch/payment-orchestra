...
fn domain_to_model(sub: &Subscription) -> Result<SubModel, SubscriptionError> {
    ...
    Ok(SubModel {
        ...
        current_period_start: sub.current_period_start.into(),
        current_period_end: sub.current_period_end.into(),
        ...
        created_at: sub.created_at.into(),
        cancelled_at: sub.cancelled_at.map(Into::into),
        paused_at: sub.paused_at.map(Into::into),
        resumed_at: sub.resumed_at.map(Into::into),
    })
}
...
fn model_to_domain(m: SubModel) -> Result<Subscription, SubscriptionError> {
    Ok(Subscription {
        ...
        resumed_at: m.resumed_at.map(Into::into),
    })
}
