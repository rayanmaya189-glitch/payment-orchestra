//! In-memory Subscription Billing repository — BC-08
...
impl SubscriptionRepository for InMemorySubscriptionRepository {
    ...
    async fn find_active_for_renewal(&self) -> Result<Vec<Subscription>, SubscriptionError> {
        ...
        let active: Vec<Subscription> = map
            .values()
            .filter(|s| s.status == SubscriptionStatus::Active || s.status == SubscriptionStatus::PastDue)
            .cloned()
            .collect();
        Ok(active)
    }
    ...
    async fn find_by_customer(&self, customer_id: Uuid) -> Result<Vec<Subscription>, SubscriptionError> {
        let map = self.subscriptions.read().await;
        let results: Vec<Subscription> = map.values().filter(|s| s.customer_id == customer_id).cloned().collect();
        Ok(results)
    }

    async fn find_by_operator(&self, operator_id: Uuid) -> Result<Vec<Subscription>, SubscriptionError> {
        let map = self.subscriptions.read().await;
        let results: Vec<Subscription> = map.values().filter(|s| s.operator_id == operator_id).cloned().collect();
        Ok(results)
    }
}
