...
use crate::infrastructure::adapters::PostgresAnalyticsRepository;
#[tokio::main]
async fn main() {
    ...
    let repo = PostgresAnalyticsRepository::new(db.clone());
    let service = AnalyticsServiceImpl::new(Box::new(repo), db.clone());
    ...
    let shutdown = platform_middleware::shutdown_signal(platform_middleware::ShutdownConfig::standard("analytics-service"));
