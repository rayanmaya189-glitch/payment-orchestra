...
use crate::infrastructure::adapters::PostgresDisputeRepository;
use crate::infrastructure::repository::PostgresEvidenceRepository;
...
async fn main() {
    ...
    let repo = PostgresDisputeRepository::new(db.clone());
    let evidence_repo = PostgresEvidenceRepository::new(db.clone());
    let service = DisputeServiceImpl::new(Box::new(repo), Box::new(evidence_repo), db.clone());
