...
pub struct AiAssistantServiceImplWithDeps {
    query_repo: Box<dyn QueryRepository>,
...
impl AiAssistantServiceImplWithDeps {
    pub fn new(
        query_repo: Box<dyn QueryRepository>,
        vector_search: Box<dyn VectorSearch>,
        llm_provider: Box<dyn LlmProvider>,
...
#[async_trait]
impl AiAssistantService for AiAssistantServiceImplWithDeps {
    async fn query(&self, cmd: AiQueryCommand) -> Result<AiQueryResponse, PlatformError> {
        let start = std::time::Instant::now();
        let mut ai_query = AiQuery::new(cmd.principal_id, cmd.query_text.clone(), cmd.session_id);

        // Step 1: Vector search for relevant context
        let search_results = self.vector_search.search(&cmd.query_text, 5).await?;
        ...
        match self.llm_provider.complete(messages, "qwen3").await {
            Ok(answer) => {
                let citations: Vec<Citation> = search_results.iter().map(|r| Citation {
                    source_type: CitationSource::from_str(&r.source_type),
                    source_id: r.source_id.clone(),
