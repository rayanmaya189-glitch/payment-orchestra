...
impl DocumentRepository for PostgresDocumentRepository {
    async fn find_by_id(&self, id: Uuid) -> Result<Option<Document>, PlatformError> {
        let m = document_entity::Entity::find_by_id(id)
            .one(&self.db)
            .await
    ...
    async fn find_by_operator(
        ...
        let models = document_entity::Entity::find()
            .filter(document_entity::Column::OperatorId.eq(operator_id))
            .limit(limit as u64)
    ...
    async fn save(&self, d: &Document) -> Result<(), PlatformError> {
        let existing = document_entity::Entity::find_by_id(d.document_id)
            .one(&self.db)
            .await
            .map_err(|e| PlatformError::Internal(format!("DB: {e}")))?;
    ...
    async fn count_by_operator_and_type(
        ...
        let models = document_entity::Entity::find()
            .filter(document_entity::Column::OperatorId.eq(operator_id))
            .filter(document_entity::Column::DocumentType.eq(document_type))
            .all(&self.db)
            .await
            .map_err(|e| PlatformError::Internal(format!("DB: {e}")))?;
...
impl From<document_entity::Model> for Document {
    fn from(m: document_entity::Model) -> Self {
        let status = DocumentStatus::from_str(&m.status).unwrap_or(DocumentStatus::Uploaded);
        let verification_status =
            VerificationStatus::from_str(&m.verification_status).unwrap_or(VerificationStatus::Pending);
        ...
        Document {
            ...
            verified_by: None, // not persisted in current schema
