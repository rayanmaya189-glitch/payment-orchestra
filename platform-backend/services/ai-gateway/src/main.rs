...
use crate::infrastructure::adapters::PostgresAiRequestRepository;

#[tokio::main]
async fn main() {
    ServiceLogger::init("ai-gateway");

    let config = AppConfig::from_env_or_panic("ai-gateway");

    let db = infrastructure::database::connect(&config.database).await;
    let redis = infrastructure::cache::connect(&config.redis).await;

    let repo = PostgresAiRequestRepository::new(db.clone());
    let service = AiGatewayServiceImpl::new(Box::new(repo), db.clone());
    ...
    let shutdown_signal = platform_middleware::shutdown_signal(
        platform_middleware::ShutdownConfig::standard("ai-gateway"),
    );
