...
pub struct PostgresNotificationRepository { db: DatabaseConnection }
...
impl NotificationRepository for PostgresNotificationRepository {
    async fn find_by_id(&self, id: Uuid) -> Result<Option<Notification>, PlatformError> {
        let m = notification_entity::Entity::find_by_id(id).one(&self.db).await
            .map_err(|e| PlatformError::Internal(format!("DB error: {e}")))?;
        Ok(m.map(|m| m.into()))
    }
    async fn save(&self, n: &Notification) -> Result<(), PlatformError> {
        let existing = notification_entity::Entity::find_by_id(n.notification_id).one(&self.db).await
            .map_err(|e| PlatformError::Internal(format!("DB error: {e}")))?;
        if let Some(model) = existing {
            ...
            let a = notification_entity::ActiveModel {
                notification_id: Set(n.notification_id), operator_id: Set(n.operator_id),
                notification_type: Set(n.notification_type.as_str().to_string()),
                status: Set(n.status.as_str().to_string()), recipient: Set(n.recipient.clone()),
                subject: Set(n.subject.clone()), body: Set(n.body.clone()),
                template_id: Set(n.template_id.clone()),
                template_data: Set(n.template_data.as_ref().and_then(|v| serde_json::to_value(v).ok())),
                ...
                created_at: Set(n.created_at.into()), updated_at: Set(n.updated_at.into()),
            };
...
impl From<notification_entity::Model> for Notification {
    fn from(m: notification_entity::Model) -> Self {
        Notification {
            notification_id: m.notification_id, operator_id: m.operator_id,
            ...
            template_data: m.template_data.and_then(|v| serde_json::from_value(v.into()).ok()),
            provider_message_id: m.provider_message_id,
            retry_count: m.retry_count, max_retries: 3,
