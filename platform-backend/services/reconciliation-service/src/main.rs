...
async fn main() {
    ...
    let repo = PostgresSettlementRepository::new(db.clone());
    let service = ReconciliationServiceImpl::new(Box::new(repo), db.clone());
    ...
    let shutdown = platform_middleware::shutdown_signal(platform_middleware::ShutdownConfig::standard("reconciliation-service"));
