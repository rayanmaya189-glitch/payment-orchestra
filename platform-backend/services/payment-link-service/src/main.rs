...
use crate::infrastructure::adapters::PostgresPaymentLinkRepository;

#[tokio::main]
async fn main() {
    ServiceLogger::init("payment-link-service");
    let config = AppConfig::from_env_or_panic("payment-link-service");
    let db = infrastructure::database::connect(&config.database).await;
    let redis = infrastructure::cache::connect(&config.redis).await;
    let repo = PostgresPaymentLinkRepository::new(db.clone());
    let service = PaymentLinkServiceImpl::new(Box::new(repo), db.clone());
    ...
    let shutdown = platform_middleware::shutdown_signal(platform_middleware::ShutdownConfig::standard("payment-link-service"));
